...
std::size_t ISerialPort::write(std::span<const uint8_t> data, std::size_t size) {
    return rawwrite(data, size);
}


std::size_t ISerialPort::read(std::span<uint8_t> buffer, std::size_t size) {
    std::size_t total = 0;
    ...
    if (fromBuffer > 0)
    {
        ...
        catch (const RingBufferUnderflowException& e) {
        }
...
std::size_t ISerialPort::peek(std::span<uint8_t> buffer, std::size_t size) {
...
std::size_t ISerialPort::drop(std::size_t size) {
...
void ISerialPort::flushPeek() {
    ringBuffer = RingBuffer<uint8_t, 1024>{};
}

void  ISerialPort::flush() {
    flushPeek();
    flushBuss();
}